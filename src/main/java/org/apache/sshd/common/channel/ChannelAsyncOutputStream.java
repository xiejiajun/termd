...
//            }
            // TODO 这样做只是为了临时解决输出太频繁时，由于session.writePacket是异步回调
            //  pendingWrite.compareAndSet(future, null)，这就会导致某些输出因pendingWrite还未成功置为null而失败
            while (pendingWrite.compareAndSet(null, future)) {}
